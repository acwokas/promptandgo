...
-- Total target: ~505 votes with unique IP addresses
WITH vote_data AS (
  SELECT 
...
sequential_votes AS (
  SELECT 
    poll_id,
    option_id,
    -- Create sequential numbering across all options to ensure unique IPs
    ROW_NUMBER() OVER (ORDER BY option_id, vote_seq) as global_seq
  FROM vote_data
),
time_distributed_votes AS (
  SELECT 
    ...
    (CASE 
      WHEN global_seq <= 76 THEN random() * INTERVAL '8 days'      -- 15% in first 8 days
      WHEN global_seq <= 177 THEN INTERVAL '8 days' + random() * INTERVAL '8 days'    -- 20% in middle 8 days  
      WHEN global_seq <= 328 THEN INTERVAL '16 days' + random() * INTERVAL '8 days'   -- 30% in next 8 days
      ELSE INTERVAL '24 days' + random() * INTERVAL '6 days'   -- 35% in last 6 days
    END) as created_at,
    -- Generate unique IP addresses using global sequence
    ('192.' || (168 + (global_seq / 65536)::int) || '.' || 
     ((global_seq / 256) % 256)::int || '.' || (global_seq % 256)::int)::inet as ip_address
  FROM sequential_votes
)
...
sequential_votes AS (
  SELECT 
    poll_id,
    option_id,
    -- Create sequential numbering starting from 1000 to avoid IP conflicts with first poll
    1000 + ROW_NUMBER() OVER (ORDER BY option_id, vote_seq) as global_seq
  FROM vote_data
),
time_distributed_votes AS (
  SELECT 
    ...
    (CASE 
      WHEN global_seq <= 1071 THEN random() * INTERVAL '7 days'     -- 12% in first 7 days
      WHEN global_seq <= 1189 THEN INTERVAL '7 days' + random() * INTERVAL '9 days'     -- 20% in next 9 days
      WHEN global_seq <= 1354 THEN INTERVAL '16 days' + random() * INTERVAL '7 days'    -- 28% in next 7 days
      ELSE INTERVAL '23 days' + random() * INTERVAL '5 days'     -- 40% in last 5 days
    END) as created_at,
    -- Generate unique IP addresses using offset global sequence to avoid conflicts
    ('10.' || ((global_seq / 65536) % 256)::int || '.' || 
     ((global_seq / 256) % 256)::int || '.' || (global_seq % 256)::int)::inet as ip_address
  FROM sequential_votes
)
INSERT INTO poll_votes (poll_id, option_id, created_at, ip_address)
